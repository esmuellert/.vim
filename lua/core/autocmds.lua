...
local function auto_update_config_repo()
  ...
  vim.system({ "git", "-C", config_path, "fetch", "--quiet" }, {}, function(fetch_result)
    ...
    -- Check ahead count (also async — can't use vim.fn in fast context)
    vim.system(
      { "git", "-C", config_path, "rev-list", "--count", "HEAD.." .. upstream },
      { text = true },
      function(ahead_result)
        if ahead_result.code ~= 0 then
          return
        end

        local ahead_count = tonumber(vim.trim(ahead_result.stdout or "0")) or 0
